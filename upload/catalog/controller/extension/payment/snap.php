...
class ControllerExtensionPaymentSnap extends Controller {
  ...
  public function process_order() {
   ...
   // $this->load->model('extension/checkout');
    ...
    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],$this->config->get('payment_snap_status_failure'));
  ...
  public function landing_redir() {
    ...
    if($payment_type == "bca_klikpay"){
     // error_log('masuk bca klikpay');
      ...
      $this->model_checkout_order->addOrderHistory($response->order_id,$this->config->get('payment_snap_status_failure'));
      $this->cart->clear();
      ...
      switch ($payment_type) {
        case "bank_transfer":
          ...
          if($check->transaction_status == "settlement"){
              ...
              $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],$this->config->get('payment_snap_status_success'));
              $this->cart->clear();
              $redirUrl = $this->url->link('checkout/success&');
              $this->response->redirect($redirUrl);
  ...
  */
  public function failure() {
   ...
   // error_log('error: snap_checkout_failure');
    unset($this->session->data['order_id']);
  ...
  public function payment_notification() {
    ...
    if ($transaction == 'capture') {
      ...
      if ($fraud == 'challenge') {
        ...
        $logs .= 'accept ';
        $this->model_checkout_order->addOrderHistory(
            $notif->order_id,$this->config->get('payment_snap_status_success'),'Update from Snap Nofif capture');
      }
    }
    else if ($transaction == 'cancel') {
        $logs .= 'cancel ';
        $this->model_checkout_order->addOrderHistory(
            $notif->order_id,$this->config->get('payment_snap_status_failure'),'Update cancel from snap notif.');
    }
    else if ($transaction == 'deny') {
      $logs .= 'deny ';
      $this->model_checkout_order->addOrderHistory(
          $notif->order_id,$this->config->get('payment_snap_status_failure'),'Update Deny from snap notif.');
    }
      ...
      $logs .= 'expire';
      $this->model_checkout_order->addOrderHistory(
          $notif->order_id,$this->config->get('payment_snap_status_failure'),'Update Expire from snap notif.');
    }
    else if ($transaction == 'failure') {
      $logs .= 'failure';
      $this->model_checkout_order->addOrderHistory(
          $notif->order_id,$this->config->get('payment_snap_status_failure'),'Update failure from snap notif');
    }
      ...
      $this->model_checkout_order->addOrderHistory(
          ...
          if($payment_type != 'credit_card'){
              $logs .= 'complete ';
              $this->model_checkout_order->addOrderHistory(
              $notif->order_id,$this->config->get('payment_snap_status_success'),'Update settlement from snap notif.');
          }
    }
